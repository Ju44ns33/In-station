...
const char *password = "profesores2022";
      ...
      if (client.available()) {             // Datos disponibles para ser leido
        ...
        if (c == '\n') {                    // Si el byte es un caracter de nuevo salto de linea
          ...
          if (currentLine.length() == 0) {    // Si no hay caracteres, entonces lanza el codgo HTML
            ...
            // Creación de botones 
            client.print("<body><h1>Temperatura</h1>");

            if(p=="off"){
              client.println("<button type='button' class='button button1' onClick=location.href='/LED=ON'>encendido</button><br><br>");                          
                }
            else{
              client.println("<button type='button' class='button button2' onClick=location.href='/LED=OFF'>apagado</button><br><br>");                                               
              }
